...
//#include "./efilter/CommonStructDefine_filter.h"

class PACKETIO_CLASS CEFilterDataSource : public CEtherDataSourceBase
{
public:
//	CEFilterDataSource(EN_FILTER_WORK_MODE enMode = EN_FILTER_WORK_MODE_MONITOR);
//	EN_FILTER_WORK_MODE m_enMode;
	CEFilterDataSource(DWORD enMode = 1);
	DWORD m_enMode;

	virtual ~CEFilterDataSource(void);


	BOOL init(int forbit_drv_load_type,const char * sysfilename,const char *drivername,const char * macaddr,
		unsigned int memsize,BOOL bEnableTimer,int nTimeStampType,BOOL bErrorRecover,
		int * error_code);

	BOOL init(int forbit_drv_load_type,const char * sysfilename,const char *drivername,int macindex_array,
		unsigned int memsize,BOOL bEnableTimer,int nTimeStampType,BOOL bErrorRecover,
		int * error_code);
