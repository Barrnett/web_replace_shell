...
BOOL CTS_BackComLan::f_ForwardIp(PACKET_INFO_EX& packet_info, ACE_UINT32 des_index)
{
...
BOOL CTS_BackComLan::f_MulticastIp(PACKET_INFO_EX& packet_info)
{	
	m_pTsSerialInterface->m_piSerialDataSend->f_SendPacket(packet_info.packet->pPacket, packet_info.packet->nPktlen, m_nComIndex);
...
BOOL CTS_BackComLan::f_ForwardArp(PACKET_INFO_EX& packet_info, ACE_UINT32 des_index)
{
	if (ECHO_OK != m_vtPeerDct[des_index]->m_eHostStat)
	{ 
		return FALSE; 
